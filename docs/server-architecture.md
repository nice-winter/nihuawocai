...
- `room:get_rooms` / `room:create` / `room:join` / `room:leave` / `room:sit` / `room:seat_open_change` / `room:password_change` / `room:lobby_broadcast` / `room:invite` / `room:game_start` / `room:quick_match`
